  ...
  L.views.calendar = {
    ...
    render(p, q) {
      ...
      for (let i = 0; i < 42; i++) {
        ...
        const ev = [
          ...sess.map(({ course: c, session: s }) => `<a class="cal-ev" style="--ch:${c.hue}" href="#/course/${c.id}/week/${s.week}" title="${esc(c.title)} · ${esc(s.kind)}">${hm(s.start)} ${esc(c.code)}${s.attended ? ' ✓' : ''}</a>`),
      ...
      const cols = DAYS.map((name, di) => {
        ...
        const blocks = R().sessionsOn(d).map(({ course: c, session: s }) => `<a class="tt-block" style="--ch:${c.hue};top:${((s.start / 60) - H0) * PX}px;height:${(s.minutes / 60) * PX - 2}px" href="#/course/${c.id}/week/${s.week}" title="${esc(c.title)}"><b>${esc(c.code)}</b>${hm(s.start)} · ${esc(s.kind)}</a>`);
      ...
      return `<div class="page">
        ...
        <div class="section"><div class="section-head"><h2>Timetable · week of ${L.fmt.date(weekStart)}</h2><span class="small muted">Concurrent courses never share a slot</span></div>
      ...
      return `<div class="page">
        ...
        <div class="transcript">
          ...
          <table class="table"><thead><tr><th>Code</th><th>Course</th><th class="num">Credits</th><th>Term</th><th class="num">Mark</th><th class="num">Grade</th></tr></thead><tbody>
            ${courses.length ? courses.map((c) => { const st = R().courseState(c); const sd = R().standing(c); return `<tr><td class="code" style="--ch:${c.hue}">${esc(c.code)}</td><td>${esc(c.title)}<div class="small muted">${esc(c.level)}</div></td><td class="num">${c.credits}</td><td class="mono small">${L.fmt.date(c.term.start)} – ${L.fmt.date(c.term.end)}</td><td class="num">${c.final ? (c.final.pct == null ? '—' : L.fmt.pct(c.final.pct)) : (sd.current == null ? '—' : L.fmt.pct(sd.current) + '*')}</td><td class="num"><span class="letter" style="font-size:18px">${c.final ? c.final.letter : (st === 'upcoming' ? '·' : 'IP')}</span></td></tr>`; }).join('') : '<tr class="row-muted"><td colspan="6">No courses on record.</td></tr>'}
          </tbody></table>
